  ...
  override def receive: Receive = {
    case CreateEvent(name, tickets) =>
      def create(): Unit = {
        ...
        val newTickets = (1 to tickets).toVector.map(TicketSeller.Ticket)
        eventTickets ! TicketSeller.Add(newTickets)
        sender() ! EventCreated(Event(name, tickets))
      }
      // TODO: Why cannot I use `Funciton.const(sender() ! EventExists)` as the last lambda function
    ...
    case GetEvents =>
      ...
      def convertToEvents(f: Future[Iterable[Option[Event]]]) =
        f.map(ito => Events(ito.flatten.toVector))
